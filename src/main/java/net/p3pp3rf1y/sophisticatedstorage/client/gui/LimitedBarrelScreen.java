...
public class LimitedBarrelScreen extends StorageScreen {
	...
	public static void updateSlotPositions(AbstractContainerMenu menu, int slotNumber, int imageWidth) {
		...
		if (slotNumber == 1) {
			...
			Slot slot = menu.getSlot(0);
			((SlotAccessor) slot).setX(halfWidth - 9);
			((SlotAccessor) slot).setY(MIDDLE_OF_STORAGE_SLOTS - 18);

			slot = menu.getSlot(1);
			((SlotAccessor) slot).setX(halfWidth - 9);
			((SlotAccessor) slot).setY(MIDDLE_OF_STORAGE_SLOTS);
			...
			slot = menu.getSlot(1);
			((SlotAccessor) slot).setX(halfWidth - 18);
			((SlotAccessor) slot).setY(MIDDLE_OF_STORAGE_SLOTS);

			slot = menu.getSlot(2);
			((SlotAccessor) slot).setX(halfWidth);
			((SlotAccessor) slot).setY(MIDDLE_OF_STORAGE_SLOTS);
